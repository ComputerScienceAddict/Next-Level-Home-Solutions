 ...
 * | Key | Purpose |
 * |-----|---------|
 * | `nlhs_detect_area_v1` | Caches the last `/api/detect-area` JSON for 24h so we don’t hammer IP/geo APIs on every navigation. |
...
export const NLHS_STORAGE = {
  DETECT_AREA_CACHE: 'nlhs_detect_area_v1',
